...
const groupAnagramsSteps: Step[] = [
  ...
  },
  {
    ...
    code: [
      { t: 'def groupAnagrams(strs):\n    result = ' },
      { blank: 0, label: '___', answer: '{}' },
      { t: '\n    for s in strs:\n        key = ' },
      { blank: 1, label: '___', answer: '"".join(sorted(s))' },
      { t: '\n        result' },
      { blank: 2, label: '___', answer: '.setdefault(key, [])' },
      { t: '.append(s)\n    return ' },
      { blank: 3, label: '___', answer: 'list(result.values())' },
    ],
    tokens: [
      '{}', '"".join(sorted(s))', '.setdefault(key, [])', 'list(result.values())',
      'set()', 'sorted(s)', 'result[key]=[]', 'result.keys()',
    ],
    feedback: {
      ok:  "Clean. setdefault(key, []) initializes the list on first encounter and returns it — no separate if-check needed. result.values() gives all groups.",
      bad: "result[key]=[] would overwrite existing groups. result.keys() returns keys, not the grouped word lists — you want result.values().",
    },
  },
  {
    type:   'complexity',
